...
from perf_review import (
    OpenAIClient, LLMJudge, MultiJudge, Arena, ELOSystem, 
    OptimizerFactory, Tournament, Submission,
    TruthAgent, TruthGuardedArena
)
    ...
    strategies = [
        {
            ...
            "focus_areas": ["Quantifiable metrics", "Data-driven results", "Performance indicators", "Impact measurement"]
        },
        {
            "name": "Leadership", 
            ...
            "focus_areas": ["Team leadership", "Mentoring", "Cross-functional collaboration", "Influence and consensus building"]
        },
        {
            "name": "Strategic",
            ...
            "focus_areas": ["Business alignment", "Strategic thinking", "Long-term planning", "Organizational impact"]
        },
        {
            "name": "Technical",
            ...
            "focus_areas": ["Technical depth", "System design", "Code quality", "Engineering excellence"]
        },
        {
            "name": "Impact",
            ...
            "focus_areas": ["Customer impact", "Revenue generation", "Cost optimization", "Market value creation"]
        },
        {
            "name": "Growth",
            ...
            "focus_areas": ["Continuous learning", "Skill development", "Innovation", "Adaptability"]
        },
        {
            "name": "Clarity",
            ...
            "focus_areas": ["Clear communication", "Professional structure", "Narrative flow", "Readability"]
        }
    ]
...
def create_leaderboard_df(tournament: Tournament) -> pd.DataFrame:
    ...
    for submission, rating in leaderboard:
        ...
        data.append({
            ...
            "Win Rate": f"{(rating.wins/rating.matches_played*100):.1f}%" if rating.matches_played > 0 else "N/A",
...
def main():
    ...
    * [View the code on GitHub](https://github.com/sshh12/perf-review)
    ...
    num_rounds = st.sidebar.slider("Number of Rounds", 1, 5, config["tournament"]["num_rounds"])
    matches_per_round = st.sidebar.slider("Matches per Round", 5, 20, config["tournament"]["matches_per_round"])
    num_agents = st.sidebar.slider("Number of Optimizer Agents", 1, len(config["agents"]), 10, 
                                  help=f"Randomly select agents from {len(config['agents'])} available strategies")
    ...
    with tab1:
        run_optimization_interface("performance_review", config, num_rounds, matches_per_round, num_agents, use_truth_agent, truth_threshold)
    
    with tab2:
        run_optimization_interface("resume", config, num_rounds, matches_per_round, num_agents, use_truth_agent, truth_threshold)


def run_optimization_interface(mode: str, config: Dict[str, Any], num_rounds: int, matches_per_round: int, 
                             num_agents: int, use_truth_agent: bool, truth_threshold: float):
    ...
    if f'original_content_{mode}' not in st.session_state:
        st.session_state[f'original_content_{mode}'] = examples[mode_config["content_example_key"]]
    if f'rubric_content_{mode}' not in st.session_state:
        st.session_state[f'rubric_content_{mode}'] = examples[mode_config["rubric_example_key"]]
    if f'background_content_{mode}' not in st.session_state:
        st.session_state[f'background_content_{mode}'] = examples[mode_config["background_example_key"]]
    
    # Single example loading button
    example_label = "📋 Load Example Data" if mode == "performance_review" else "📄 Load Resume Example"
    if st.button(example_label, help=f"Load all example data for {mode.replace('_', ' ')}", key=f"load_example_{mode}"):
        st.session_state[f'original_content_{mode}'] = examples[mode_config["content_example_key"]]
        st.session_state[f'rubric_content_{mode}'] = examples[mode_config["rubric_example_key"]]
        st.session_state[f'background_content_{mode}'] = examples[mode_config["background_example_key"]]
        st.rerun()
    ...
    st.caption("Optional supporting data to prevent AI from fabricating claims (recommended)")
    ...
    if st.button("🚀 Run AI Optimization", type="primary", key=f"run_btn_{mode}"):
        ...
        try:
            ...
            if use_truth_agent and background.strip():
                submissions_to_verify = [s for s in completed_tournament.submissions if s.id != "original"]
                status_text.text(f"🔍 Fact-checking {len(submissions_to_verify)} submissions...")
                ...
                if blocked_count > 0:
                    status_text.text(f"🛡️ Truth verification complete: {blocked_count} submissions blocked")
                    st.warning(f"⚠️ Truth agent blocked {blocked_count} submissions for potential fabrications")
                else:
                    status_text.text(f"✅ Truth verification complete: All {len(approved_submissions)} submissions approved")
                time.sleep(0.5)
            ...
            # Display results
            st.success(f"🎉 Tournament completed! {len(completed_tournament.submissions)} submissions, {len(completed_tournament.matches)} matches")
            ...
            st.header("🥇 Winning Submission")
            display_submission_details(winner_submission)
            ...
            if "No module named" in error_msg:
                st.error("❌ Missing required dependency. Please check your installation.")
            elif "API key" in error_msg.lower():
                st.error("❌ OpenAI API key issue. Please check your OPENAI_API_KEY environment variable.")
            elif "rate limit" in error_msg.lower():
                st.error("❌ OpenAI API rate limit reached. Please wait a moment and try again.")
            elif "timeout" in error_msg.lower():
                st.error("❌ Request timed out. The AI models may be experiencing high demand. Please try again.")
            elif len(error_msg) == 36 and "-" in error_msg:  # Likely a UUID error
                st.error("❌ Internal processing error. This may be due to a configuration issue with the selected agents. Please try running again.")
            else:

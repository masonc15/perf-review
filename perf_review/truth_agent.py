...
class TruthAgent:
    ...
    def verify_submission(
        self, submission: Submission, ground_truth: str, rubric: str
        ...
        if not function_calls:
            raise ValueError(
                "Truth agent did not provide structured verification result"
            )
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            ...
            for submission in submissions:
                future = executor.submit(
                    self._verify_single, submission, ground_truth, rubric
                )
            ...
            for future in as_completed(future_to_submission):
                ...
                try:
                    ...
                    if result.is_truthful:
                        print(
                            f"✅ Truth verification passed: {submission.agent_name or 'Original'}"
                        )
                    else:
                        print(
                            f"❌ Truth verification failed: {submission.agent_name or 'Original'}"
                        )
                except Exception as e:
                    print(
                        f"⚠️ Truth verification error for {submission.agent_name or 'Original'}: {e}"
                    )
...
Use the verify_truthfulness function to provide your structured analysis."""
    ...
    def _create_verification_prompt(
        self, submission: Submission, ground_truth: str
    ) -> str:
...
class TruthGuardedArena:
    ...
    def __init__(
        self, truth_agent: TruthAgent, min_truthfulness_threshold: float = 0.7
    ):
        """
    ...
    def filter_submissions(
        ...
        """
        verification_results = self.truth_agent.batch_verify(
            submissions, ground_truth, rubric, batch_size
        )

        approved_submissions = []
        for submission, verification in zip(submissions, verification_results):
            if (
                verification.is_truthful
                and verification.confidence >= self.min_threshold
            ):

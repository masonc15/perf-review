...
class Arena:
    ...
    def run_optimization_round(
        ...
        for optimizer in optimizers:
            for attempt in range(max_attempts_per_agent):
                ...
                optimization_tasks.append((optimizer, parent_submission, tournament.rubric))
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            ...
            for optimizer, parent_submission, rubric in optimization_tasks:
                future = executor.submit(self._run_single_optimization, optimizer, parent_submission, rubric)
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    if submission:
                        ...
                        tournament.add_submission(submission)  # This creates the rating too
                        optimizer.attempts_made += 1
                        print(f"✅ {optimizer.strategy.name}: Created optimized submission")
                    else:
                        print(f"⚠️  {optimizer.strategy.name}: Failed to create submission")
                except Exception as e:
                    print(f"❌ {optimizer.strategy.name}: Optimization failed: {e}")
        
        return new_submissions
    
    def _run_single_optimization(self, optimizer: OptimizerAgent, parent_submission: Submission, rubric: str) -> Optional[Submission]:
    ...
    def run_tournament_matches(
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            ...
            future_to_pair = {}
            for submission_a, submission_b in pairs:
                future = executor.submit(self._run_single_match, submission_a, submission_b, tournament.rubric)
            ...
            for future in as_completed(future_to_pair):
                ...
                try:
                    ...
                    if match:
                        ...
                        print(f"⚖️ Match completed: {submission_a.agent_name or 'Original'} vs {submission_b.agent_name or 'Original'}")
                    else:
                        print(f"⚠️ Match failed: {submission_a.agent_name or 'Original'} vs {submission_b.agent_name or 'Original'}")
                except Exception as e:
                    print(f"❌ Match error: {e}")
        
        return new_matches
    
    def _run_single_match(self, submission_a: Submission, submission_b: Submission, rubric: str) -> Optional[Match]:
    ...
    def _get_round_robin_pairs(self, submissions: List[Submission]) -> List[Tuple[Submission, Submission]]:
    ...
    def _get_swiss_pairs(self, tournament: Tournament) -> List[Tuple[Submission, Submission]]:
    ...
    def _print_leaderboard(self, tournament: Tournament, top_n: int = 10) -> None:
        ...
        print("-" * 80)
        print(f"{'Rank':<5} {'Rating':<8} {'W-L':<8} {'Agent':<15} {'Confidence':<12} {'Preview'}")
        print("-" * 80)
        ...
        for rank, (submission, rating) in enumerate(leaderboard[:top_n], 1):
            ...
            print(f"{rank:<5} {rating.rating:<8.0f} {win_loss:<8} {agent_name:<15} {confidence:<12} {preview}")
...
class OptimizerAgent:
    ...
    def optimize_submission(
        ...
        if self.attempts_made > self.strategy.max_attempts:
            raise ValueError(f"Agent {self.name} has exceeded max attempts ({self.strategy.max_attempts})")
        ...
        tools = [{
            ...
            "parameters": {
                ...
                "required": ["optimized_content", "improvements_made", "rationale", "confidence_level", "areas_needing_feedback"],
    ...
    def optimize_with_feedback_loop(
        ...
        for iteration in range(max_iterations):
            ...
            if iteration < max_iterations - 1:  # Don't judge on last iteration
                ...
                judge_result = judge.judge_single(original_submission, optimized, rubric)
    ...
    def _create_optimizer_system_prompt(self, rubric: str) -> str:
        ...
        focus_areas_text = "\n".join([f"- {area}" for area in self.strategy.focus_areas])
